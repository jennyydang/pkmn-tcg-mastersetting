...
import { getCardsForSet, getSet } from "@/lib/api";
...
export default function SetPage({ params }: Props) {
  ...
  const { trackedSets, addSet, isCardOwned } = useStore();

  const tracked = trackedSets.find((t) => t.set.id === id);
  const ownedCount = tracked?.ownedCards.length ?? 0;
  const total = set ? (set.printedTotal ?? set.total) : 0;
  ...
  return (
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              {set?.series} · Released {set?.releaseDate} · {total} cards
            </p>
      ...
      {filteredCards.length > 0 ? (
        <CardGrid cards={filteredCards} setId={id} />
      ) : (
